...
class CartRuleCore extends ObjectModel
{
	...
	//zjisti, zda je voucher pro slevomat validni
	function checkSlevomat($voucherCode)
	{
		$token = ""; //doplnit token drinxperience
	...
	//zjisti, zda je voucher s postovnym nebo bez
	function checkVoucherFreeShipping($voucherCode)
	{
		$slevomatShippingCode = substr($voucherCode, 9, 2); //pro extrakci kodu = 0B nebo 0A
		if($slevomatShippingCode == "0A")
		{
			return false; //bez postovneho
		}
		if($slevomatShippingCode == "0B")
		{
			return true; //s postovnym 
		}
		
		
	}
	
	function addSlevomatVoucher($voucherCode)
	{
	...
	public function checkValidity(Context $context, $alreadyInCart = false, $display_error = true)
	{
		...
		//implement slevomat here	
		$slevomatResult = checkSlevomat($this->code);
		if($slevomatResult)
		{
			if(checkVoucherCode($this->code))
			{
				//vztvoreni voucheru s postovnym
			}
			else
			{
				//vytvoreni voucheru bez postovneho
			}
		}
